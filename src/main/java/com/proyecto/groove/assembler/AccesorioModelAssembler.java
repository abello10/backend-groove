...
public class AccesorioModelAssembler implements RepresentationModelAssembler<Accesorio,EntityModel<Accesorio>>{
    ...
    public EntityModel<Accesorio> toModel(Accesorio accesorio){
        return EntityModel.of(accesorio,
            linkTo(methodOn(AccesorioControllerV2.class).getAccesorioById(accesorio.getId())).withSelfRel(),
            linkTo(methodOn(AccesorioControllerV2.class).getAllAccesorios()).withRel("accesorios"),
            linkTo(methodOn(AccesorioControllerV2.class).updateAccesorio(accesorio.getId(),accesorio)).withRel("actualizar"),
            linkTo(methodOn(AccesorioControllerV2.class).deleteAccesorio(accesorio.getId())).withRel("eliminar"),
            linkTo(methodOn(AccesorioControllerV2.class).partialUpdateAccesorio(accesorio.getId(), accesorio)).withRel("reemplazar una línea")
        );
    }
}